...
public class ChatHub(UserManager<AppUser> userManager, AppDbContext context) : Hub
{
    ...
    public async Task  LoadMessages(string recipienId, int pageNumber = 1){
        int pageSize = 10;
        var username = Context.User!.Identity!.Name;
        var currentUser = await userManager.FindByNameAsync(username!);
        ...
        List<MessageResponseDto> messages = await context.Messages.Where(x => (x.ReceiverId == currentUser.Id &&
         x.SenderId == recipienId) || (
         x.SenderId == currentUser.Id &&
          x.ReceiverId == recipienId))
    ...
    public async Task SendMessage(MessageRequestDto message){
        ...
        await Clients.User(receiverId!).SendAsync("ReceiverNewMessage", newMsg);
